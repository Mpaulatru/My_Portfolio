...
export default function HeroWaveBackground() {
    ...
    function draw() {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          const wave =
            Math.sin(r * 0.18 + time * 0.6 + c * 0.06) * 7 +
            Math.sin(c * 0.12 - time * 0.4) * 3
          const x = baseX + wave
          const shade =
            0.08 +
            0.14 * (0.5 + 0.5 * Math.sin(r * 0.2 + c * 0.15 + time * 0.6))
          ctx.beginPath()
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className="absolute inset-0 w-full h-full [mask-image:radial-gradient(ellipse_65%_55%_at_50%_0%,black,transparent)]"
    />
  )
}
